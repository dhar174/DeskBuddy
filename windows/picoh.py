...
eyeShapeRight = ''
eyeShapeFile = ''
phonemeFile = 'picohData/phonemes.txt'
speechAudioFile = 'picohData/picohspeech.wav'
input_speech = 'picohData/input_speech.wav'
...
# String to hold location of settings file
settingsFile = 'picohData/PicohSettings.xml'
speechGender = "Male"
cogServicesID = ''
...
if not path.exists(speechDatabaseFile):
    shutil.copyfile(os.path.join(directory, 'PicohSpeech.csv'),
                    speechDatabaseFile)
    if debug:
...
if not path.exists(picohMotorDefFile):
    shutil.copyfile(os.path.join(
        directory, 'MotorDefinitionsPicoh.omd'), picohMotorDefFile)
    if debug:
        print("Copied MotorDefinitionsPicoh.omd from :" +
              directory + " to picohData/")
...
def _generateSpeechFile(text):
    ...
    if platform.system() == "Windows":
        if ("sapi" in synthesizer.lower()):
            ...
            with open(phonemeFile, mode='wb') as f:
                for sent in sentences(safetext, lang="en-us"):
                    for word in sent:
                        if word.phonemes:
                            for (char) in word.phonemes:
                                f.write(bytes(char, 'utf-8'))
            url = "http://localhost:5002/api/tts"

            params = {
                "voice": "en-us/scottish_english_male-glow_tts",
                "text": str(safetext),
    ...
    if platform.system() == "Darwin":
        ...
        # Execute bash command.
        ret = subprocess.Popen(bashcommand, shell=True,
    ...
    if platform.system() == "Linux":
        ...
        if synthesizer.upper() == "GTTS":
            ...
            if synthesizer.upper() == "LARYNX":
                bashcommand = "larynx -ssml -v scottish_english_male-glow_tts" + '"' + \
                    safetext + '"> picohData/picohspeech.wav")' + \
                    "' '(utt.save.segs mytext " + '"picohData/phonemes"' + ")'"
            else:
                bashcommand = synthesizer + ' -w picohData/picohspeech.wav ' + \
                    voice + ' "' + safetext + '"'
...
def GenerateSsml(textToSpeak, locale, gender, voiceName):
    ...
    ssmlDoc += "<voice xml:lang='" + locale + "' xml:gender='" + \
        gender + "' name='" + locale + "-" + voiceName + "'>"
    ssmlDoc += textToSpeak
    ssmlDoc += "</voice>"
    ssmlDoc += "</speak>"
...
def move(m, pos, spd=5, eye=0):
    ...
    if (motorType[m] == "Matrix X"):
        ...
        if eye == 1 or eye == 0:
            msg = "FE,2," + \
                "{:0.0f}".format(pos * 255 / 10) + "," + \
                "{:0.0f}".format(lastfeyl * 255 / 10) + "\n"
...
def getTimings(speechAudioFile, phonemes):
    ...
    write(input_speech, fs, myrecording)
...
def get_input_path():
    ...
    # Save the recording to a WAV file
    # sf.write(input_speech, myrecording, fs)
    return os.path.abspath(input_speech)


def say(text, untilDone=True, lipSync=True, hdmiAudio=True, soundDelay=0):
    global topLipFree
        ...
        if vals:
            for i in range(0, len(vals)):
                ...
                for j in range(0, len(vals[i])):
                    ...
                    phonemes.append(vals[0])
    # print("len of phenomes[] "+str(len(phonemes)))
    times = getTimings(speechAudioFile, phonemes)
    ...
    if lipSync:
        if soundDelay > 0:
            ...
            t = threading.Timer(soundDelay, _playSpeech,
                                args=(hdmiAudio,), kwargs=None)
            # Set up a thread for the speech movement
            t2 = threading.Thread(target=_moveSpeech,
                                  args=(phonemes, times, True))
        else:
            ...
            t2 = threading.Timer(-soundDelay, _moveSpeech,
                                 args=(phonemes, times, True), kwargs=None)
        t2.start()
    else:
        # Set up a thread for the speech sound synthesis
        t = threading.Thread(target=_playSpeech, args=(hdmiAudio,))
        t2 = threading.Timer(-soundDelay, _moveSpeech,
                             args=(phonemes, times, False), kwargs=None)
        t2.start()
    t.start()
    setBaseColor(0, 0, 5)
...
def _playSpeech(addSilence):
    ...
    if platform.system() == "Linux":
        if addSilence:
            commandString = 'aplay -D plug:default ' + \
...
def _setEyes(leftDefinition, rightDefinition="", autoMirror=True):
    ...
    _serwrite("FB,0," + _EyeShapeBytes(definition,
              rightDefinition, 0, autoMirror) + "\n")
    _serwrite("FB,1," + _EyeShapeBytes(definition,
              rightDefinition, 1, autoMirror) + "\n")
    _serwrite("FB,2," + _EyeShapeBytes(definition,
              rightDefinition, 2, autoMirror) + "\n")
    _serwrite("FB,3," + _EyeShapeBytes(definition,
              rightDefinition, 3, autoMirror) + "\n")
    _serwrite("FB,4," + _EyeShapeBytes(definition,
              rightDefinition, 4, autoMirror) + "\n")
    # Pupil is held in set 6 and has been implemented in the Arduino driver as FB 8
    _serwrite("FB,8," + _EyeShapeBytes(definition,
              rightDefinition, 5, autoMirror) + "\n")
